...
export const DonationForm = (props) => {
  ...
  const stepTwo = () => {
    return <article className="step-two">
        ...
        {errors.cardNumber && <span>⚠️ Card number is required and should be 16 digits long</span>}
        <input {...register("expDate", {required: true, pattern: /^(0[1-9]|1[0-2])\/?([0-9]{2})$/})} placeholder="Expiration Date MM/YY"/>
        {errors.expDate && <span>⚠️ Expiration date doesn't match the format.</span>}
        <input {...register("CVV", {required: true, maxLength: 3, minLength: 3})} placeholder="CVV"/>
        {errors.CVV && <span>⚠️ CVV is a 3 digit number</span>}
  ...
  if (currentStep !== 3) {
    ...
    return (
      <form className="donation-form" onSubmit={handleSubmit(onSubmit)}>
        <header>
          <h3>{props.title || "Donate Today!"}</h3>
          <img src={props.logo} alt={props.title + " logo"} className="logo"/>
        </header>
        <article className="step-one">
          <input placeholder="First Name" {...register("firstName", { required: true })} />
          {errors.firstName && <span>⚠️ First Name is required</span>}
          <input placeholder="Last Name" {...register("lastName", { required: true })}/>
          {errors.lastName && <span>⚠️ Last Name is required</span>}
          <input {...register("email", { required: true })} type="email" placeholder="email"/>
          {errors.email && <span>⚠️ Email is required</span>}
    ...
    return <div className="submitted-form"> 
      ...
      <p>You can expect to receive a confirmation email at shortly.</p>
      <p></p>
